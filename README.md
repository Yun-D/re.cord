...
## 💡 기술 선택 이유
### Next.js를 사용하지 않은 이유
re.cord는 개인 기록형 서비스로, 검색 엔진 노출이 필요하지 않다고 생각했습니다.
SSR/SSG의 이점이 유효한 서비스(블로그, 콘텐츠 플랫폼 등)와 달리 re.cord는 로그인한 사용자만 자신의 데이터를 보는 구조이므로 순수 React SPA로 구성하는 것이 적합하다고 판단했습니다.

### Firebase (Authentication + Firestore)
별도 백엔드 서버 없이 인증과 DB를 모두 처리할 수 있어 선택했습니다.
방문 기록을 누적해서 쌓아가는 서비스 특성상 데이터가 유실되면 서비스 자체가 의미를 잃기 때문에 외부 DB 저장이 필요했습니다. 
Firebase는 익명 인증만으로도 사용자별 데이터를 분리해 관리할 수 있어 이 서비스의 방향성에 적합했습니다.

### CSS Modules (Styled-Components에서 전환)
이전 프로젝트(왓포스트)에서는 Styled-Components를 사용했으나,
이후 CSS-in-JS 방식이 런타임에 스타일을 생성하는 특성상
초기 렌더링 성능에 영향을 줄 수 있다는 점을 학습하여 전환을 택했습니다.

이 프로젝트에서는 스타일을 두 레이어로 분리해 관리했습니다.
- **글로벌 CSS**: 컬러, 폰트 사이즈, 보더 등 디자인 토큰을 CSS 변수로 정의하고, 전역에서 공통으로 쓰이는 레이아웃, 텍스트 클래스를 함께 관리
- **CSS Modules**: 공통 컴포넌트 스타일을 하나의 모듈 파일로 관리, 클래스명 충돌 방지

### Vercel
GitHub 연동 시 push만으로 자동 배포되는 환경을 무료로 제공해
개인 프로젝트 배포 및 지속적인 업데이트에 적합하다고 판단했습니다.
